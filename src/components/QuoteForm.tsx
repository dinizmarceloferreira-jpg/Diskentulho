...
const TIPOS = [
  ...
  "Gesso",
  "Terra e Pedras",
  "Poda / Entulho de Jardim",
  "Misto",
  "Outro",
];

const inputBase =
  "w-full px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl border border-slate-800 bg-slate-900/90 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-sm min-w-0";
...
export default function QuoteForm() {
  ...
  if (submitted) {
    return (
      <div className="bg-slate-950 rounded-3xl border border-slate-800 p-6 sm:p-8 md:p-10 shadow-xl text-center">
        <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-emerald-950 border border-emerald-500/30 flex items-center justify-center mx-auto mb-4 sm:mb-5">
  ...
  const Field = ({
    ...
    <div className="min-w-0">
      ...
      {children}
      {error && (
        <p role="alert" className="mt-1 sm:mt-1.5 text-xs font-medium text-red-400 flex items-center gap-1">
    ...
    >
      ...
      <div className="grid sm:grid-cols-2 gap-4 sm:gap-5">
        <Field name="tamanho" label="Tamanho da Caçamba" required error={errors.tamanho}>
          <select
            ...
            className={`${inputBase} ${errors.tamanho ? "border-red-400" : "border-slate-800"}`}
          >
      ...
      <div>
        <label className="block text-xs sm:text-sm font-semibold text-slate-300 mb-1.5 sm:mb-2">
          Foto do Local ou Entulho <span className="text-slate-500 font-normal">(opcional)</span>
        </label>
        {photoPreview ? (
      ...
      <div>
        <label className="flex items-start gap-2.5 sm:gap-3 cursor-pointer group">
          <input
            ...
            className="mt-0.5 w-4 h-4 sm:w-5 sm:h-5 rounded border-slate-800 bg-slate-900 text-emerald-600 focus:ring-emerald-500/30 cursor-pointer flex-shrink-0"
          />
          <span className="text-xs sm:text-sm text-slate-400 leading-relaxed">
            Autorizo o tratamento dos meus dados pessoais conforme a{" "}
            <span className="font-semibold text-slate-200">LGPD (Lei nº 13.709/2018)</span> para fins de contato e envio de orçamento pela Itauna Locacao de Equipamentos LTDA. *
          </span>
        </label>
        {errors.lgpd && (
          <p role="alert" className="mt-1.5 text-xs font-medium text-red-400 flex items-center gap-1">
            <span className="w-1 h-1 rounded-full bg-red-400" />
            {errors.lgpd}
          </p>
        )}
